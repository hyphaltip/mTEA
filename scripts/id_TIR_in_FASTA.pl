...
print "fasta_title\tTIR1_start\tTIR1_end\tTIR2_start\tTIR2_end\tTIR1_sequence\tTIR2_sequence\ta_possible_TSD1\ta_possile_TSD2\n";

#read input fasta files, assuming ORF boundaries are provided in the fasta title, if not sequence is split into to equal halves
...
while (my $seq = $infasta->next_seq) {
	...
	my %tsd; #holds an index number as key, as value has an array with the following informating for each possible tsd pair: [0] sequence on the left side of the sequence, [1] sequence on the right, [2] start position on left, [3] start postion on right, [4] acceptable pair or not, boolean, 0 for not acceptable
	...
	my $count_uc_standard_char = ($seq->seq() =~ tr/ACGTN//); #number of uppercase standard characters
	my $count_lc_standard_char = ($seq->seq() =~ tr/ACGTN//); #number of lowercase standard characters
	...
	my $temp_seq1_filename = File::Temp->new( UNLINK => 0, SUFFIX => '.fas' );
	my $temp_seq2_filename = File::Temp->new( UNLINK => 0, SUFFIX => '.fas' );
	my $temp_blast_filename = File::Temp->new( UNLINK => 0, SUFFIX => '.blast' );
	...
	#at the moment I don't know a good way to capture errors from blastn, seems like redirecting the output to 2>&1 is not working
	...
	while (<$fh>) {
		my($q, $s, $p_identities, $identities, $unk, $gaps, $q_start, $q_end, $s_start, $s_end, $e, $score) = split;
		next if ($s_start < $s_end); #only look at blast plus/minus hits for tirs
		next if ((abs($s_end - $s_start) < $MIN_TIR_LENGTH) || (abs($q_end - $q_start) < $MIN_TIR_LENGTH)); #skip if TIR is too short
		...
		foreach my $ltsd (@leftp_tsds) {
			foreach my $rtsd (@rightp_tsds) {
				my($test_tsd_outcome,$ltsd_used, $rtsd_used) = testsds($ltsd, $rtsd, $seq->seq()); #test for possible identidy between tsds
				if($test_tsd_outcome) { 
					...
					my $tir1_seq = substr($seq->seq(),$ltsd+(length $TSD_CONSENSUS)+1, $q_end - $ltsd - (length $TSD_CONSENSUS) - 1);
					my $tir2_seq = substr($seq->seq(),($s_end + (length $seq1)), $rtsd - ($s_end + (length $seq1)));

					print "$fastatitle\t$tir1_pos_start\t$tir1_pos_end\t$tir2_pos_start\t$tir2_pos_end\t$tir1_seq\t$tir2_seq\t$ltsd_used\t$rtsd_used\n";
				}
			}		
		}		
	}
...
sub testsds {
	...
	unless ($TSD_INSERTIONS < 1) {
		for (my $i=0; $i<length $TSD_CONSENSUS; $i++) {
			$tsd1{substr($tsd1_seq, 1, $i) . "-" . substr($tsd1_seq, $i + 1, (length $TSD_CONSENSUS) - $i - 1)} = 1;
			$tsd2{substr($tsd2_seq, 1, $i) . "-" . substr($tsd2_seq, $i + 1, (length $TSD_CONSENSUS) - $i - 1)} = 1;		
		}
	}

	#generate TSDs with deletions
	unless (($TSD_DELETIONS < 1) || ($tsd1p < $TSD_DELETIONS) || ($tsd2p > (length ($seq) - (length $TSD_CONSENSUS) - $TSD_DELETIONS)))  { #ignore if no deletions were requested or if the TSD is too close to the ends of the sequence to make deletion
